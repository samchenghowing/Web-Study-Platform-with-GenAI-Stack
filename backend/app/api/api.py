...
[ Quiz ]
3.  `/quiz/{id}`:                   Fetches quiz questions for a student from MongoDB or loads default questions if none are found.
4.  `/quiz/generate`:               Generates a quiz using the LLM.
5.  `/submit/quiz`:                 Streams responses from an LLM to check quiz answers.
6.  `/submit/settings`:             Updates user settings in Neo4j based on quiz submissions.
7.  `/bgtask/{uid}/status`:         Retrieves the status of a background task.
8.  `/submit`:                      Submits a background task to verify a submission.

[ User Record ]
9.  `/login/`:                      (Read) Authenticates a user by email and password.
10. `/signup/`:                     (Creates) a new student record in MongoDB and initializes data in Neo4j.
11. `/check_new_student/{user_id}`: (Read) Checks if a user is logging in for the first time.
12. `/students/`:                   (Read) Lists up to 1000 student records from MongoDB.
13. `/students/{id}`:               (Read) Retrieves details of a specific student.
14. `/students/{id}` (PUT):         (Updates) fields of a specific student.
15. `/students/{id}` (DELETE):      (Deletes) a specific student record.

[ PDF ]
16. `/upload/pdf`:                  Uploads PDFs, generates thumbnails, stores them in MongoDB GridFS, and triggers a background task.
17. `/pdfs`:                        Lists uploaded PDFs and their thumbnails.
18. `/pdfs/{id}`:                   Deletes a specific PDF record from MongoDB.

[ Outside Soure ]
19. `/load/stackoverflow`:          Loads StackOverflow data in the background.
20. `/load/website`:                Loads website data in the background.

[ Chat history ] 
21. `/chat_histories/{SessionId}`:  Fetches chat history for a given session.
22. `/chat_histories/{SessionId}`:  Deletes chat history for a session.
